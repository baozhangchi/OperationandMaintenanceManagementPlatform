...
public sealed partial class CpuMonitor
{
    ...
    private async Task<ChartDataSource> OnInit()
    {
        ...
        using (var client = new HttpClient())
        {
            try
            {
                var responseMessage = await client.GetAsync($"{ClientApiUrl}/api/Cpu/1000");
                var responseContent = await responseMessage.Content.ReadAsStringAsync();
                var data = JsonConvert.DeserializeObject<List<CpuLog>>(responseContent);
